...
bool testChessPrintLevelsAndTournamentStatistics(){
    FILE* file_levels = fopen("../cmake-build-debug/your_output.txt", "w");
    if(!file_levels){
    ...
    ASSERT_TEST(chessSaveTournamentStatistics(chess, "../cmake-build-debug/tournament_statistics_your_output.txt") == CHESS_SUCCESS);
    chessDestroy(chess);
    fclose(file_levels);
    return true;
}
